...
DEFAULT_EXCLUDE_MODELS_TRACKING = (
    "quartet_capture.models.Task",
    "quartet_capture.models.TaskHistory",
    "quartet_capture.models.TaskMessage",
)

tracked_app_list = getattr(settings, "AUDIT_TRACKED_APPS", DEFAULT_AUDIT_TRACKED_APPS)

exclude_models = getattr(
    settings, "EXCLUDE_MODELS_TRACKING", DEFAULT_EXCLUDE_MODELS_TRACKING
)
...
def _history_user_setter(historical_instance, user):
    if user is not None:
        historical_instance.history_user_id = "{0}:{1}".format(user.username, user.pk)
...
class HistoricalRecords(sh_models.HistoricalRecords):
    def __init__(
        ...
        history_user_id_field = models.CharField(
            null=True, verbose_name="username", max_length=200
        )
        super().__init__(
    ...
    for app_name in app_list:
        ...
        for model in list(app.models.values()):
            if get_model_fullname(model) not in exclude_models:
                register(model, app=__package__, records_class=HistoricalRecords)
